    ...
    return this.each(function(i, elem) {
      ...
      $table
        .find("thead th:nth-child(1) input[type='checkbox']")
          .on("change", function() {
            if ($(this).is(":checked")) {
              $table.find("tbody td:nth-child(1) input[type='checkbox']:not(:checked)").prop("checked", true).trigger("change");
            } else {
              $table.find("tbody td:nth-child(1) input[type='checkbox']:checked").prop("checked", false).trigger("change");
            }
          })
          .end()
        .find("tbody tr")
        ...
        .find("tbody td:nth-child(1) input[type='checkbox']")
          .on("change", function() {
            var $uncheckedCheckboxes = $(this).closest("tbody").find("td:nth-child(1) input[type='checkbox']:not(:checked)"),
